...
struct BookingView: View {
    ...
    @ObservedObject var connectAssembly = HookahAssemblyModel.shared
    @ObservedObject var connectCatalog = CatalogViewModel.shared
            ...
            .popup(isPresented: $clickSelectedTable, type: .floater(verticalPadding: 0, useSafeAreaInset: true), position: .bottom, animation: .spring(), dragToDismiss: true, closeOnTap: false, dismissCallback: {
                ...
                ZStack{
                    VStack{
                        ...
                        Button(action: {
                            print("SEND")
                            connectNetwork.sendOrder(order: connectAssembly.hookahs, food: connectCatalog.cartFood, user: connectNetwork.userProfile, date: date, table: connectBooking.selectedTable.id)
